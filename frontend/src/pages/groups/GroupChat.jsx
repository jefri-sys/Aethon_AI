...
const GroupChat = ({ group, onLeave }) => {
  ...
  const renderMessageContent = (msg) => {
    ...
    if (messageType === 'summary') {
      return (
        <>
          <div className="flex items-center text-amber-700 mb-2 border-b border-amber-200/60 pb-2 text-xs font-sans font-bold tracking-wide uppercase">
  ...
  return (
    <div className="flex flex-col h-full w-full">
      ...
      <div className="bg-white border-b border-slate-200 p-4 flex justify-between items-center shadow-sm z-10 shrink-0">
        <div 
          className="cursor-pointer hover:bg-slate-50 p-2 -ml-2 rounded-lg transition"
          onClick={() => setShowSettingsModal(true)}
        >
          <div className="flex items-center space-x-2">
            <h2 className="text-lg font-bold text-slate-800">{group.name}</h2>
            {!group.isPublic && (
              <span className="bg-slate-100 text-slate-500 text-[10px] px-2 py-0.5 rounded-full uppercase font-bold tracking-wide">Private</span>
            )}
          </div>
          <p className="text-xs text-slate-500">{group.members?.length || 1} members • {group.course}</p>
        </div>
        <div className="flex space-x-2">
          <button 
            onClick={handleCopyLink}
            className="flex items-center space-x-1.5 px-3 py-1.5 bg-indigo-50 text-indigo-600 border border-indigo-200 rounded-md hover:bg-indigo-100 transition text-sm font-medium"
          ...
          <button 
            ...
            className="flex items-center space-x-1.5 px-3 py-1.5 bg-amber-50 text-amber-700 border border-amber-200 rounded-md hover:bg-amber-100 transition text-sm font-medium"
          ...
          <button 
            ...
            className="flex items-center px-2 py-1.5 bg-rose-50 text-rose-600 rounded-md hover:bg-rose-100 transition text-sm"
            title="Leave Group"
          >
            ...
            if (msg.messageType === 'system') {
              return (
                <div key={msg._id || idx} className="flex justify-center my-2">
                  <div className="bg-slate-200/50 text-slate-500 text-xs px-3 py-1 rounded-full text-center max-w-[80%] shadow-sm">
                    {msg.message}
                  </div>
                </div>
              );
              ...
              return (
                <div key={msg._id || idx} className="flex flex-col items-start mb-2">
                  <div className="flex items-center ml-1 mb-1">
                    <span className="flex items-center text-[10px] text-indigo-700 font-bold tracking-wide uppercase bg-indigo-100 px-2 py-0.5 rounded-full">
                      <Zap className="w-3 h-3 mr-1 text-indigo-500 fill-current" /> Synapse AI
                    </span>
                    <span className="ml-2 text-[9px] uppercase font-bold tracking-wider text-slate-400">
                      • {badge}
                    </span>
                  </div>
                  <div className="max-w-[95%] sm:max-w-[85%] p-4 shadow-sm bg-indigo-50 border border-indigo-200 text-slate-800 rounded-2xl rounded-tl-none prose prose-sm prose-indigo">
                    <ReactMarkdown>{msg.message}</ReactMarkdown>
                    <div className="text-[10px] mt-2 flex items-center justify-end text-slate-400 font-sans">
            ...
            return (
              <div key={msg._id || idx} className={`flex flex-col ${isMine ? 'items-end' : 'items-start'} group`}>
                ...
                <div className="flex items-center">
                  {!isMine && (
                    <button 
                      ...
                      className={`opacity-0 group-hover:opacity-100 transition-opacity mr-2 p-1.5 rounded-full flex-shrink-0 ${evalQuestionId === msg._id ? 'bg-amber-100 text-amber-600 opacity-100' : 'bg-slate-100 hover:bg-indigo-100 text-slate-400 hover:text-indigo-600'}`}
                  ...
                  <div 
                    ...
                    isSummary 
                      ? 'bg-amber-50/80 border border-amber-200 text-slate-800 rounded-2xl rounded-tl-none font-serif relative overflow-hidden' 
                      : isMine 
                ...
                >
                  ...
                  <div className={`text-[10px] mt-2 flex items-center justify-end space-x-1 ${isMine && !isSummary ? 'text-indigo-200' : 'text-slate-400'}`}>
                    <Clock className="w-3 h-3" />
                    <span>{new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    {isMine && (
                      <span className="ml-1">
                        {isRead ? <CheckCheck className="w-3.5 h-3.5 text-blue-300" /> : <Check className="w-3 h-3 text-indigo-300" />}
                      </span>
                    )}
                  </div>
                </div>
                {isMine && (
                  <button 
                    onClick={() => handleEvaluate(msg._id)}
                    className={`opacity-0 group-hover:opacity-100 transition-opacity ml-2 p-1.5 rounded-full flex-shrink-0 ${evalQuestionId === msg._id ? 'bg-amber-100 text-amber-600 opacity-100' : 'bg-slate-100 hover:bg-indigo-100 text-slate-400 hover:text-indigo-600'}`}
                    title={evalQuestionId === msg._id ? "Question selected" : (evalQuestionId ? "Evaluate this answer" : "Select as question")}
                  >
                    <Zap className="w-3.5 h-3.5" />
                  </button>
      ...
      {showNotebookModal && (
        <div className="absolute inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-5 max-h-[80%] flex flex-col">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-slate-800">Share Summary</h3>
              <button onClick={() => setShowNotebookModal(false)} className="text-slate-400 hover:text-slate-600">
                <X className="w-5 h-5" />
              </button>
            </div>
            <p className="text-xs text-slate-500 mb-4">Select a notebook. The latest AI summary from its chat will be shared with the group.</p>
            <div className="flex-1 overflow-y-auto space-y-2">
              {notebooks.length === 0 ? (
                <div className="text-center py-6 border border-dashed border-slate-200 rounded-lg">
                  <p className="text-sm text-slate-500">No notebooks available.</p>
                </div>
              ) : (
                notebooks.map(notebook => (
                  <button 
                    ...
                    className="w-full text-left p-3 border border-slate-200 rounded-lg hover:bg-indigo-50 hover:border-indigo-200 transition group flex items-center"
                  >
                    <BookOpen className="w-5 h-5 text-slate-400 group-hover:text-indigo-500 mr-3 shrink-0" />
                    <div className="overflow-hidden">
                      <div className="font-semibold text-slate-800 text-sm truncate">{notebook.title}</div>
                      <div className="text-xs text-slate-500 truncate">{notebook.subjectId?.name || "No Subject"}</div>
                    </div>
                  </button>
                ))
              )}
