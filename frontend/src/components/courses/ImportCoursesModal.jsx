...
export default function ImportCoursesModal({ isOpen, onClose, onImportSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-5xl bg-slate-900 h-full flex flex-col shadow-2xl animate-in slide-in-from-right duration-300">
        
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-800 p-6">
          <div>
            <h2 className="text-2xl font-bold text-white">Import Courses</h2>
            <p className="text-slate-400 text-sm mt-1">Bulk add your academic schedule from Excel, CSV, or PDF.</p>
          </div>
          <button onClick={handleClose} className="p-2 text-slate-400 hover:text-white rounded-full hover:bg-slate-800 transition-colors">
            <X size={24} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
            ...
            <div className="space-y-4">
              ...
              >
                ...
                <div className="flex flex-col items-center justify-center space-y-4">
                  <div className={`p-4 rounded-full ${dragActive ? 'bg-indigo-500/20 text-indigo-400' : 'bg-slate-800 text-slate-400'}`}>
                    <UploadCloud size={32} />
                  </div>
                  <div>
                  ...
                  <button className="px-4 py-2 mt-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-sm font-medium transition-colors border border-slate-700">
                    Browse Files
                  </button>
                </div>
              </div>
          ...
          {file && !loadingAI && (
            <div className="flex items-center justify-between p-4 bg-slate-800/50 border border-slate-700 rounded-xl">
              ...
              <button 
                onClick={resetState}
                className="text-slate-400 hover:text-white text-sm font-medium px-3 py-1.5 rounded-md hover:bg-slate-700 transition-colors"
              >
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="text-sm text-slate-400">
                  ...
                  <span className="text-emerald-400">{validCount} valid</span> &bull; {' '}
                  <span className={invalidCount > 0 ? "text-red-400 font-medium" : ""}>{invalidCount} need attention</span>
                </div>
              </div>
                    ...
                    <tbody className="divide-y divide-slate-700/50">
                      {previewData.map((row, idx) => {
                        ...
                        return (
                          <tr 
                            key={row._id} 
                            className={`group transition-colors ${!isValid ? 'bg-red-900/10' : 'hover:bg-slate-800/40'}`}
                          >
                            ...
                            <td className="px-4 py-2">
                              <div className="flex items-center gap-2">
                                {!row.code && <AlertTriangle size={14} className="text-red-400 shrink-0" />}
                                <input 
                                  className={`bg-transparent w-full focus:ring-1 focus:ring-indigo-500 rounded px-2 py-1 outline-none transition-colors border ${!row.code ? 'border-red-500/50 text-red-300 placeholder-red-400/50' : 'border-transparent text-slate-200 hover:border-slate-700 focus:bg-slate-800'}`}
                                  value={row.code || ''}
                            ...
                            <td className="px-4 py-2">
                              <div className="flex items-center gap-2">
                                {!row.name && <AlertTriangle size={14} className="text-red-400 shrink-0" />}
                                <input 
                                  className={`bg-transparent w-full min-w-[200px] focus:ring-1 focus:ring-indigo-500 rounded px-2 py-1 outline-none transition-colors border ${!row.name ? 'border-red-500/50 text-red-300 placeholder-red-400/50' : 'border-transparent text-slate-200 hover:border-slate-700 focus:bg-slate-800'}`}
                                  value={row.name || ''}
                            ...
                            <td className="px-4 py-2">
                              <input 
                                className="bg-transparent w-full min-w-[120px] focus:ring-1 focus:ring-indigo-500 rounded px-2 py-1 outline-none border border-transparent text-slate-200 hover:border-slate-700 focus:bg-slate-800 transition-colors"
                                value={row.professor || ''}
                            ...
                            <td className="px-4 py-2">
                              <input 
                                type="number"
                                className="bg-transparent w-16 focus:ring-1 focus:ring-indigo-500 rounded px-2 py-1 outline-none border border-transparent text-slate-200 hover:border-slate-700 focus:bg-slate-800 transition-colors"
                                value={row.semester || ''}
                            ...
                            <td className="px-4 py-2">
                              <input 
                                className="bg-transparent w-full min-w-[150px] focus:ring-1 focus:ring-indigo-500 rounded px-2 py-1 outline-none border border-transparent text-slate-200 hover:border-slate-700 focus:bg-slate-800 transition-colors"
                                value={row.schedule || ''}
                            ...
                            <td className="px-4 py-2">
                              <input 
                                className="bg-transparent w-full min-w-[100px] focus:ring-1 focus:ring-indigo-500 rounded px-2 py-1 outline-none border border-transparent text-slate-200 hover:border-slate-700 focus:bg-slate-800 transition-colors"
                                value={row.room || ''}
                                placeholder="Room"
        ...
        {/* Footer */}
        {previewData.length > 0 && !loadingAI && (
          <div className="border-t border-slate-800 p-6 bg-slate-900 flex justify-between items-center shrink-0">
            ...
            <div className="flex gap-3">
              ...
              <button 
                ...
                className="px-5 py-2.5 rounded-lg font-medium bg-indigo-600 text-white hover:bg-indigo-700 focus:ring-4 focus:ring-indigo-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
                {importing ? (
